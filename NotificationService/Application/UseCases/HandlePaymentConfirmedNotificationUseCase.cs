...
public class HandlePaymentConfirmedNotificationUseCase
{
    ...
    private readonly ILogger<HandlePaymentConfirmedNotificationUseCase> _logger;

    public HandlePaymentConfirmedNotificationUseCase(IEmailSender emailSender, ICustomerClient customerClient, IConfiguration configuration, ILogger<HandlePaymentConfirmedNotificationUseCase> logger)
    {
    ...
    public async Task Execute(PaymentConfirmedEvent paymentEvent, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            {
                To = customerEmail,
                Subject = "Seu boleto fictício foi confirmado",
                Body = htmlBody
            };

            await _emailSender.SendEmailAsync(email, cancellationToken);
            _logger.LogInformation($"Notification email sent for confirmed payment {paymentEvent.PaymentId}");
        }
        catch (Exception ex)
        {
            _logger.LogError($"Error sending notification email for confirmed payment {paymentEvent.PaymentId}: {ex.Message}");
            throw;
        }
    }

    private string GenerateEmailHTML(string paymentId, string customerName, string orderID)
    {
        return $@"
<!DOCTYPE html>
<html lang=""pt-BR"">
...
<body>
    <div class=""container"">
        <div class=""header"">
            <h1>💳 Boleto Fictício Confirmado</h1>
        </div>
        ...
        <div class=""content"">
            ...
            <p style=""color: #999; font-size: 12px; margin-top: 30px; font-style: italic;"">
                Este é um boleto fictício apenas para fins de demonstração.
            </p>
        </div>
