...
public class HandlePaymentCancelledNotificationUseCase
{
    ...
    private readonly ILogger<HandlePaymentCancelledNotificationUseCase> _logger;

    public HandlePaymentCancelledNotificationUseCase(IEmailSender emailSender, ICustomerClient customerClient, IConfiguration configuration, ILogger<HandlePaymentCancelledNotificationUseCase> logger)
    {
    ...
    public async Task Execute(PaymentCancelledEvent paymentEvent, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            {
                To = customerEmail,
                Subject = "Seu boleto fictício foi cancelado",
                Body = htmlBody
            };

            await _emailSender.SendEmailAsync(email, cancellationToken);
            _logger.LogInformation($"Notification email sent for canceled payment {paymentEvent.PaymentId}");
        }
        catch (Exception ex)
        {
            _logger.LogError($"Error sending notification email for canceled payment {paymentEvent.PaymentId}: {ex.Message}");
            throw;
        }
    }
...
<body>
    <div class=""container"">
        <div class=""header"">
            <h1>💳 Boleto Fictício Cancelado</h1>
        </div>
